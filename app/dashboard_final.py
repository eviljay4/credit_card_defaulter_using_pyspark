...
def db_setup_and_loader(uploaded_file):
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload your credit_card_transactions.csv", type=["csv"])
    if not st.session_state.db_ready:
        if uploaded_file is not None:
            if st.button("Run DB Setup and Loader"):
                msg = db_setup_and_loader(uploaded_file)
                st.session_state.db_ready = True
                st.success(msg)
                st.info("**Please refresh the page (F5 or Ctrl+R) to continue.**")
                st.stop()
        else:
            st.info("Upload the CSV file and click the button above to set up and load the database before proceeding.")
            st.stop()
    else:
